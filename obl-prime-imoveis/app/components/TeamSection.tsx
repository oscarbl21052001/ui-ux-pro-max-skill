...
export default function TeamSection() {
  ...
  // Title — leads everything, fully landed by 30% scroll
  const titleScale   = useTransform(smooth, [0, 0.3], [2.0, 1]);
  const titleZ       = useTransform(smooth, [0, 0.3], [380, 0]);
  const titleOpacity = useTransform(smooth, [0, 0.15], [0, 1]);
  const titleBlurN   = useTransform(smooth, [0, 0.25], [8, 0]);
  const titleFilter  = useTransform(titleBlurN, (v) => `blur(${v}px)`);

  // Polaroid — one beat behind title, landed by 35%
  const polaroidScale   = useTransform(smooth, [0.05, 0.35], [2.6, 1]);
  const polaroidZ       = useTransform(smooth, [0.05, 0.35], [420, 0]);
  const polaroidOpacity = useTransform(smooth, [0.05, 0.2], [0, 1]);
  const polaroidBlurN   = useTransform(smooth, [0.05, 0.3], [6, 0]);
  const polaroidFilter  = useTransform(polaroidBlurN, (v) => `blur(${v}px)`);

  // Text block — subtlest depth, two beats behind, landed by 40%
  const textScale   = useTransform(smooth, [0.1, 0.4], [1.5, 1]);
  const textZ       = useTransform(smooth, [0.1, 0.4], [260, 0]);
  const textOpacity = useTransform(smooth, [0.1, 0.3], [0, 1]);
  const textBlurN   = useTransform(smooth, [0.1, 0.35], [5, 0]);
